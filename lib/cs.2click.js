...
function flagsOn() {
	...
	tsk = new Task(flagsOff, this);
	tsk.interval = 100; // every 100ms
	tsk.repeat(1);  // do it once
}

// Here is a repeater function that posts its
// iteration count to the Max window:

flagsOff.local = 1;
function flagsOff() {
	twoclickFlags.set(modulename, 0);
}
